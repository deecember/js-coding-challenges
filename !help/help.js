...
Die geschweiften Klammern definieren dabei den Bereich der Bedingung, wenn diese erfüllt wird. Alles was außerhalb der Klammern steht, gehört nicht mehr zur If Abfrage.
...
/*DE: Am Anfang steht das Schlüsselwort "for", danach kommen runde Klammern und anschließend geschweifte Klammern.
Die geschweiften Klammern definieren dabei den Bereich, welcher jedes Mal bei einem Schleifendurchlauf ausgeführt wird. Alles was außerhalb der Klammern steht, gehört nicht mehr zur Schleife.
Innerhalb der runden Klammern, musst du nun deine Bedienung festlegen. Wie lange die Schleife ausgeführt werden soll. Dafür kannst du die obigen Operatoren verwenden.
Also definieren wir eine Variable i welche 0 ist.
Anschließend schreiben wir die Bedingung, welche nicht mehr erfüllt werden darf, damit die Schleife beendet wird (solange i kleiner als 10). Und zum Schluss schreiben wir was passiert,
wenn ein Schleifendurchlauf fertig ist. In diesem Fall soll die Variable i um 1 erhöht werden. Getrennt werden die 3 Blöcke mit einem ";". 

Die Ausgabe von dieser Schleife wären die Zahlen 0 - 9.

EN: At the beginning there is the keyword "for", followed by round brackets and then curly brackets.
The curly brackets define the area that is executed each time the loop is run. Everything outside the brackets is no longer part of the loop.
Inside the round brackets, you have to define your operation. How long the loop should be executed. For this you can use the operators above.
So we define a variable i which is 0.
Then we write the condition, which must not be fulfilled anymore, so that the loop is terminated (as long as i is smaller than 10). And finally we write what happens
when a loop is finished. In this case the variable i should be incremented by 1. The 3 blocks are separated with a ";". 

The output of this loop would be the numbers 0 - 9.*/
for(let i = 0; i < 10; i++){
    console.log(i);
}